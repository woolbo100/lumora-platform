...
export default async function AdminFreebiesPage() {
  ...
  try {
    const response = await supabaseRestRequest("freebies?order=sort_order.asc,created_at.desc");
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div className="space-y-2">
          ...
          <p className="text-sm text-[var(--foreground-muted)]">
            사용자에게 제공할 무료 PDF 자료를 등록하고 관리합니다.
          </p>
        </div>
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-white/5 bg-white/2 text-xs font-semibold tracking-wider text-[var(--foreground-muted)] uppercase">
                ...
                <th className="px-6 py-4">썸네일 / 제목</th>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-[var(--foreground-muted)]">/{item.slug}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
