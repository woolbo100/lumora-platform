...
export default async function NamingResultPage({ searchParams }: NamingResultPageProps) {
  ...
  const validated = validateNamingInput({
    ...
    purpose: getSingleValue(params.purpose) as "wealth" | "love" | "brand" | "healing" | undefined,
  ...
  let result = null;
  try {
    result = generateNamingResult(validated.data);
  } catch {
    result = null;
  }
  ...
  if (!result) {
    return (
      <main className="mx-auto flex min-h-screen w-full max-w-7xl flex-col px-6 py-12 sm:px-8 lg:px-12">
        <ErrorState errors={["사주 공통 분석 데이터를 찾을 수 없어요. 사주 결과 화면에서 다시 이어서 시작해주세요."]} />
      </main>
    );
  }
